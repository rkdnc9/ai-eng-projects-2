...
```mermaid
graph LR
    A[General Purpose LLM] --> B[Fine-Tuning on<br/>Document Database]
    B --> C[Specialized LLM]
...
graph TD
    ...
    B --> C[Adapter Layer<br/>Trainable]
    C --> D[Frozen Layer 2]
    D --> E[Adapter Layer<br/>Trainable]
    E --> F[Frozen Layer 3]
    F --> G[Adapter Layer<br/>Trainable]
    G --> H[Output]
```
...
```mermaid
graph LR
    A[Input x] --> B[Frozen Path:<br/>W·x]
    A --> C[LoRa Path:<br/>B·A·x]
    B --> D[Addition]
    C --> D
    D --> E[Output]
```
...
```mermaid
graph LR
    A[User Query] --> B[Prompt Engineering<br/>Logic]
    B --> C[Enhanced Prompt]
    C --> D[LLM]
    D --> E[Response]
```
...
```mermaid
graph LR
    A[System Prompt<br/>Hidden Instructions] --> C[Combined]
    B[User Prompt<br/>User Query] --> C
    C --> D[LLM]
    D --> E[Response]
```
...
```mermaid
graph LR
    A[User Query +<br/>ALL Documents] --> B[LLM]
    B --> C[Response]
```
...
```mermaid
graph LR
    A[Document Database<br/>PDFs, HTMLs, Images] --> B[Index Building<br/>Logic]
...
```mermaid
graph LR
    A[User Query] --> B[Search Logic]
    C[Searchable Index] --> B
    B --> D[Retrieved Chunks]
```
...
graph TD
    ...
    B --> C[Identify Components:<br/>Title, Text, Figure, Table]
    C --> D[Text Extraction<br/>OCR Methods]
...
```mermaid
graph LR
    A[Text: To learn more about<br/>our products, contact us] --> B[Text Embedding Model]
    B --> C[Vector: 0.23, -0.15, 0.67,<br/>..., 0.42]
```
...
```mermaid
graph TD
    A[Text Chunks] --> B[Text Embedding Model]
...
```mermaid
graph LR
    A[User Query] --> B[Text Encoder]
    B --> C[Query Embedding]
    C --> D[Search Algorithm]
    E[Index] --> D
    D --> F[Top-K<br/>Relevant Chunks]
```
...
```mermaid
graph TD
    A[All Data Points] --> B[Clustering Algorithm<br/>K-Means]
    B --> C[Cluster 1]
    B --> D[Cluster 2]
    B --> E[Cluster 3]
    B --> F[Cluster 4]
...
```mermaid
graph TD
    A[Query Point] --> B[Find Nearest<br/>Centroid]
    B --> C[Cluster 3<br/>Selected]
    C --> D[Search Within<br/>Cluster Only]
    D --> E[Return Top-K<br/>from Cluster]
```
...
```mermaid
graph TD
    A[Root: Full Space] --> B[Left Subspace]
    A --> C[Right Subspace]
    B --> D[Region 1]
    B --> E[Region 2]
    C --> F[Region 3]
...
```mermaid
graph LR
    A[Retrieved Content] --> B[Prompt Engineering]
    C[User Query] --> B
    B --> D[Enhanced Prompt]
    D --> E[LLM]
    E --> F[Response]
...
```mermaid
graph TD
    A[Create Training Data] --> B[Label Documents<br/>as Relevant/Irrelevant]
    B --> C[Fine-Tune LLM]
    C --> D[Model Learns to<br/>Identify Relevant Docs]
    D --> E[Better Generation<br/>Despite Noisy Retrieval]
```
...
graph TD
    ...
    F[Context Relevance] --> G[Query vs Context]
    H[Faithfulness] --> I[Response vs Context]
    J[Answer Correctness] --> K[Response vs Query]
```
...
```mermaid
graph TD
    A[User Query] --> B[Input Guardrails/<br/>Safety Filtering]

    B -->|Safe| C[Query Rewrite/<br/>Query Expansion]
    B -->|Unsafe| D[Rejection Response]

    C --> E[Text Encoder]
    E --> F[Query Embedding]

    G[Document Database] --> H[Document Parsing]
    H --> I[Document Chunking]
    I --> J[Text/Image Encoders]
    J --> K[Indices<br/>Text + Image]

    F --> L[Nearest Neighbor<br/>Search ANN]
    K --> L

    L --> M[Retrieved Chunks<br/>Text + Images]

    M --> N[Prompt Engineering]
    C --> N

    N --> O[Enhanced Prompt +<br/>Retrieved Context]

    O --> P[LLM<br/>General Purpose or<br/>RAFT Fine-Tuned]

    P --> Q[Generated Response]

    Q --> R[Output Guardrails/<br/>Response Safety]

    R -->|Safe| S[Display to User]
    R -->|Unsafe| D
```
